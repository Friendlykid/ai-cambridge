        ...
        for action in [
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextPosition in self.corners:
                    nextVisitedCorners[self.corners.index(nextPosition)] = True
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for order in permutations(remainingCorners):
        ...
        for corner in order:
            totalDistance += util.manhattanDistance(currentPosition, corner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    heuristic = max(util.manhattanDistance(position, food) for food in foodList)

    for index, firstFood in enumerate(foodList):
        firstDistance = util.manhattanDistance(position, firstFood)
        for secondFood in foodList[index + 1 :]:
            secondDistance = util.manhattanDistance(position, secondFood)
            pairDistance = util.manhattanDistance(firstFood, secondFood)
            heuristic = max(
                heuristic, min(firstDistance, secondDistance) + pairDistance
            )
