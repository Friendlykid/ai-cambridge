...
class PerceptronModel(Module):
    ...
    def run(self, x):
        ...
        """
        return tensordot(x, self.w, dims=([1], [1])).reshape(1)
...
class RegressionModel(Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.hidden_layer_1 = Linear(1, 128)
        self.hidden_layer_2 = Linear(128, 128)
    ...
    def train(self, dataset):
        ...
        full_x = tensor(dataset.x, dtype=torch.float32)
        full_y = tensor(dataset.y, dtype=torch.float32)
        ...
        for _ in range(2000):
            ...
            if self.get_loss(full_x, full_y).item() <= 0.015:
                break
...
class DigitClassificationModel(Module):
    ...
    def __init__(self):
        ...
        input_size = 28 * 28
        output_size = 10
        self.hidden_layer_1 = Linear(input_size, 512)
        self.hidden_layer_2 = Linear(512, 256)
    ...
    def get_loss(self, x, y):
        ...
        return -(y * torch.log(probabilities + 1e-12)).sum(dim=1).mean()
...
class LanguageIDModel(Module):
    ...
    def __init__(self):
        ...
        self.output_hidden = Linear(hidden_size, hidden_size)
    ...
    def run(self, xs):
        ...
        """
        first_step = xs[0]
        hidden = relu(self.initial_layer(first_step))

        for x in xs[1:]:
            timestep_present = (x.sum(dim=1, keepdim=True) > 0).float()
            updated_hidden = relu(self.input_layer(x) + self.hidden_layer(hidden))
            hidden = timestep_present * updated_hidden + (1 - timestep_present) * hidden

        hidden = relu(self.output_hidden(hidden))
    ...
    def train(self, dataset):
        ...
        for _ in range(40):
            for sample in dataloader:
                optimizer.zero_grad()
                xs = movedim(sample["x"], 0, 1)
                loss = self.get_loss(xs, sample["label"])
                loss.backward()
                optimizer.step()
            ...
            if validation_accuracy >= 0.84 or epochs_without_improvement >= 5:
                break
...
def Convolve(input: tensor, weight: tensor):
    ...
    for y in range(output_height):
        ...
        for x in range(output_width):
            region = input[y : y + weight_dimensions[0], x : x + weight_dimensions[1]]
            output_row.append(tensordot(region, weight, dims=2))
        output_rows.append(stack(output_row))

    Output_Tensor = stack(output_rows)
...
class Attention(Module):
    ...
    def forward(self, input):
        ...
        k_matrix = self.k_layer(input)
        q_matrix = self.q_layer(input)
        v_matrix = self.v_layer(input)
        attention_scores = matmul(k_matrix, movedim(q_matrix, 1, 2)) / (
            self.layer_size**0.5
        )
        attention_scores = attention_scores.masked_fill(
            self.mask[:, :, :T, :T] == 0, float("-inf")
        )[0]
        return matmul(softmax(attention_scores, dim=-1), v_matrix)
